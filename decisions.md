...
## 2026-06-13 09:00 MST — Visibility-Changing Supabase Endpoints Need Scoped RPCs
**Context:** Address soft delete originally used a client-side update to set `deleted_at`. Because active-address RLS excludes rows where `deleted_at` is not null, the update failed with "new row violates row-level security policy" and the delete modal stayed open.
**Decision:** Use `doorstep.soft_delete_address(...)` as a scoped `SECURITY DEFINER` RPC for address soft delete. The function checks `auth.uid()`, verifies `addresses.write` for the workspace, constrains the address to the workspace, uses a fixed `search_path`, and grants execute only to `authenticated`.
**Rationale:** Some mutations intentionally move a row out of the current RLS visibility window. Those transitions should be handled by audited endpoints with explicit permission checks before mutation instead of ordinary client updates.
**Alternatives considered:** Broaden the address update RLS `with check` to allow deleted rows; rejected because it would make general update policy harder to reason about. Keep an invoker RPC; rejected because it still hits the same RLS visibility transition.
**Consequences:** Future endpoints that alter row visibility, ownership, tenant association, or lifecycle state must be designed and documented as endpoint contracts first, including RLS interaction, grants, error behavior, and migration history.

## 2026-06-18 12:00 MST — Platform Owner Control Plane Starts With Audited Read Model
**Context:** DoorStep needs to become a true multi-workspace platform where the Platform Owner can see all workspaces, users, and usage while normal users stay constrained by workspace RLS.
**Decision:** Add a Platform Owner dashboard foundation using `doorstep.profiles.is_platform_owner`, `doorstep.platform_audit_events`, `doorstep.api_registry`, `doorstep.record_platform_audit_event(...)`, and `doorstep.platform_dashboard_overview()`. The first platform UI is read-only usage/account visibility. True impersonation is deferred to a separate backend-only, audited design.
